...
/*
Chart describing coefficient assignment for different levels of accuracy:
...
void fill(double* source, fin_dif_coef* fin);
int fin_dif_init(int a, fin_dif_coef* fin);
...
int fin_dif_init(int a, fin_dif_coef* fin){
	fin->accuracy = a + (a % 2); //This ensures that accuracy=a if a is even, but accuracy = a+1 if a is odd.
	fin->accuracy = minmax(ACC_MIN, fin->accuracy, ACC_MAX); //This ensures a is one of the hard-coded acceptible values.
...
void fill(double* source, fin_dif_coef* fin){
	fin->coef = malloc(sizeof(double)*(fin->accuracy));
	int i = 0;
	for(; i < fin->accuracy; i++){
		fin->coef[i] = source[i];
	}
}
